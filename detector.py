...
def fonk(temp_video_name):
    ...
    #loading image
    cap=cv2.VideoCapture(temp_video_name) #0 for 1st webcam
    ...
    cursor.execute("""select * from records_data order by created_at desc limit 1""")
    last_insert_data = cursor.fetchone()
